...
exports.decode=function(stream) {
	...
	map = {
		...
		0x07 : function() {return 'ObjectId('+reader.readBytesAsHexString(12)+')';},//read 12 byte of objectId
...
exports.encode= function(jsobj,writer) {
	...
	var writeDocument=function(obj,writeChildren) {
		...
		writer.writeInt32(doc.length+4);
		writer.stream+=doc;
	}
	...
	map= {
		...
		"[object String]" : function(key,value) {
			if(value.indexOf('ObjectId(')==0) {
				writer.writeByte(0x07).writeZeroTermString(key).writeHexString(value.substring(9,value.length-1));
			} else {
				writer.writeByte(0x02).writeZeroTermString(key).writeString(value);
			}
				} ,
