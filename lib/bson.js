function BinaryReader(stream) {
	...
	this.readInt64 = function() {
		...
		high = high * 0x7FFFFFFF * 2 + 1;
		return high + (high < 0 ? -1 : 1) * low;
	};
...
0x01 : 	function() {return reader.readDouble();},
	...
	0x09 :  function(){var d=new Date();d.setUTCMilliseconds(reader.readInt64());return d;},
	0x0A :  function(){return reader.readZeroTermString();},
	0x0B :  function(){return reader.readZeroTermString();},//RegExp
...
function BinaryWriter() {
	...
	this.writeInt64 = function(l) {
		this.stream+= this.writeInt32((l - (l > 0 ? 1 : -1) * (l & 0xFFFFFFFF))
				/ ((0x7FFFFFFF) * 2) - 1).writeInt32(l & 0xFFFFFFFF);
		return this;
	};
	this.writeDouble = function(d) {
		if (isNaN(d) || d == Infinity || d == -Infinity) {
			if (isNaN(d)) {
				this.writeInt32((2047 << 20) | (1 << 19)).writeInt32(0);
			} else {
				this.writeInt32((d < 0 ? -1 : 1) * (2047 << 20)).writeInt32(0);
			...
			this.writeInt32((d < 0 ? 1 << 31 : 0) | high).writeInt32(low);
			return this;
	...
	this.writeUTF8CharCode=function(ch) {
			if (ch < 128) {
					...
					this.writeByte((ch >> 6) | 192);
					this.writeByte((ch & 63) | 128);
			} else {
					this.writeByte((ch >> 12) | 224);
					this.writeByte(((ch >> 6) & 63) | 128);
					this.writeByte((ch & 63) | 128);
			}
			return this;
	};
	...
	map= {
		"[object Number]": function(key,value){ 
			if(value==Math.floor(value)) {
				...
				writer.writeByte(0x01).writeDouble(value);
			}
		},
		...
		"[object Boolean]" : function(key,value){writer.writeByte(0x08).writeBoolean(value);},
		"[object RegExp]": function(key,value){
			writer.writeByte(0x0A).writeZeroTermString(key).writeZeroTermString(value.source).writeZeroTermString(value.toString(value.substring(value.source.length+2)))
		},
		"[object Function]":function(key,value){writer.writeByte(0x0D).writeZeroTermString(key).writeString(value);},
		"[object Date]":function(key,value){writer.writeByte(0x09).writeZeroTermString(key).writeInt64(value.getUTCMilliseconds());}
...
console.dir(decodeBSON(encodeBSON({hello:"world",a:1,b:{z:3,a:"привет утф8",ar:[123,234]}})));
